...
int main(int argc, char** argv) {
  ...
  #pragma acc set device_num(0);   // Set which GPU in your environment to offload execution onto

  // OpenACC device data mapping
  #pragma acc enter data copyin(inimage[0:volume], x_d[0:volume], y_d[0:volume], z_d[0:volume])

  start = omp_get_wtime();

  if(bc == 0) {
    if(dim == 1) {
      fdiff_gpu_1d_nm(inimage, x_d, sizeX, sizeY, sizeZ, bc, T, volume);  // X derivatives
    } else if(dim == 2) {
      fdiff_gpu_2d_nm(inimage, x_d, y_d, sizeX, sizeY, sizeZ, bc, T, volume); // XY derivatives
    } else if(dim == 3) {
       fdiff_gpu_3d_nm(inimage, x_d, y_d, z_d, sizeX, sizeY, sizeZ, bc, T, volume); // XYZ derivatives
     }
  }

  else if(bc == 1) {
    if(dim == 1) {
      fdiff_gpu_1d_pd(inimage, x_d, sizeX, sizeY, sizeZ, bc, T, volume);  // X derivatives
    } else if(dim == 2) {
      fdiff_gpu_2d_pd(inimage, x_d, y_d, sizeX, sizeY, sizeZ, bc, T, volume); // XY derivatives
    }
    else if(dim == 3) {
      fdiff_gpu_3d_pd(inimage, x_d, y_d, z_d, sizeX, sizeY, sizeZ, bc, T, volume); // XYZ derivatives
    }
  }

  else {
    if(dim == 1) {
      fdiff_gpu_1d_nb(inimage, x_d, sizeX, sizeY, sizeZ, bc, T, volume);  // X derivatives
    } else if(dim == 2) {
      fdiff_gpu_2d_nb(inimage, x_d, y_d, sizeX, sizeY, sizeZ, bc, T, volume); // XY derivatives
    }
    else if(dim == 3) {
      fdiff_gpu_3d_nb(inimage, x_d, y_d, z_d, sizeX, sizeY, sizeZ, bc, T, volume); // XYZ derivatives
    }
  }

  end = omp_get_wtime();

  /* OpenACC exit data mapping */
  #pragma acc exit data copyout(x_d[0:volume], y_d[0:volume], z_d[0:volume]) delete(inimage[0:volume])
