		     ...
		     int nx, int ny, int nz, int bc, int T)
{
    ...
    int offset1 = (nz - 1) * nx * ny;      // index of beginning of last slice
    int offset2 = offset1 + (ny - 1) * nx;  // index of beginning of last row
    ...
    #pragma omp parallel num_threads(T)
    {
	    ...
	    pix0 = -inimage[ind];
	    outimageX[ind] = pix0 + inimage[ind + 1];          // Calculate X diff between x and x+1 pixels
	    outimageY[ind] = pix0 + inimage[ind + nx];         // Calculate Y diff between y and y+1 pixels
	}

        // Computation on the last row
	#pragma omp for
	...
	// Computation on the final slice
	#pragma omp for
	for(ind = 0; ind < nx - 1; ind++) {
		     ...
		     float *outimageZfull, int nx, int ny, int nz, int bc, int T)
{
    ...
    #pragma omp parallel num_threads(T)
    {
	    ...
	    outimageX[ind] = pix0 + inimage[ind + 1];          // Calculate X diff between x and x+1 pixels
	    outimageY[ind] = pix0 + inimage[ind + nx];         // Calculate Y diff between y and y+1 pixels
	    outimageZ[ind] = pix0 + inimage[ind + nx * ny];    // Calculate Z diff between z and z+1 pixels
	}
	...
	if(bc == 0) {
	  ...
	  // Periodic boundaries
	    ...
	    for(k = 0; k < nz; k++){
	        for(i = 0; i < nx; i++) {
		    int ind1 = (k * ny * nx);
		    int ind2 = ind + (ny - 1) * nx;
		    outimageY[ind2 + i] = -inimage[ind2 + i] + inimage[ind1 + 1];
		}
	    }
